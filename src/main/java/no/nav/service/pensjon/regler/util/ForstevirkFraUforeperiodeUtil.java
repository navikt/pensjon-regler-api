...
import no.nav.domain.pensjon.regler.grunnlag.PersonDetalj;
...
import no.nav.service.pensjon.regler.to.TrygdetidRequest;
...
public class ForstevirkFraUforeperiodeUtil {
	...
	//protected final static Log log = LogFactory.getLog(ForstevirkFraUforeperiodeUtil.class);
...
//			}
			for (Enumeration<Date> e = ht.keys(); e.hasMoreElements();) {
				...
				Persongrunnlag p = (Persongrunnlag)ht.get(uft);
		...
		Date uft = (Date)ht.keys().nextElement(); // kun ett element i tabellen
		for (VilkarsVedtak v : vlist) {
